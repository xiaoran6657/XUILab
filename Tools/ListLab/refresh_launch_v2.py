"""Version 2 List refresh Player dispatch with durable intents and exact owned PIDs.
...
def verify_receipt(root,run,ph,bh,player,manifest):
    ...
    need(r.get("schemaVersion")=="xuilab.list-refresh.launch-receipt/v2" and r.get("runId")==name,"Receipt schema/run")
    ...
    record=read(start)
    need(record["schemaVersion"]=="xuilab.list-refresh.launch-start/v2","Launch record schema")
    for key in ("runId","pid","planSha256","buildManifestSha256","player","playerSha256","command","launcherSha256"):
    ...
    expected={"-screen-fullscreen":"0","-screen-width":"960","-screen-height":"540","-logFile":str(log),"--xuilab-run-id":name,"--xuilab-output-root":str(root),"--refresh-plan-sha256":ph,"--refresh-build-manifest-sha256":bh}
...
def launch(plan_path,root,gate_path,player,build_path,repo):
    ...
    try:
        ...
        for run in plan["runs"]:
            ...
            marker={"schemaVersion":"xuilab.list-refresh.launch-intent/v2","runId":run_id,"planSha256":plan_hash,"buildManifestSha256":build_hash,"token":token,"ownerPid":os.getpid(),"player":str(player),"createdUtc":dt.datetime.now(dt.timezone.utc).isoformat()}
            ...
            marker.update(command=command,playerSha256=contracts.sha256_file(player),launcherSha256=contracts.sha256_file(Path(__file__)))
            ...
            try:
                ...
                contracts.write_json_new(start_path,dict(marker,schemaVersion="xuilab.list-refresh.launch-start/v2",pid=process.pid))
