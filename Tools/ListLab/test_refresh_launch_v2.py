    ...
    def exercise(self,mode):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            def result(*args):
                ...
                return dict(state='completed',correctness='pass',measurementValidity='valid'),dict(frameP95=1)
            ...
            with contextlib.ExitStack() as stack:
                ...
                if mode=='success':m.launch(plan_path,root,gate,player,build,repo)
                else:
                    with self.assertRaises(BaseException):m.launch(plan_path,root,gate,player,build,repo)
                self.assertEqual(popen.call_count,1)
            uncertain=mode in ('interrupt','kill_failed','start_write_failed','spawn_failed')
            self.assertEqual((repo/'Artifacts/list-refresh-player.lock').exists(),uncertain)
