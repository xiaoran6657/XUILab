...
import adaptive_continue as m
import adaptive_recovery
import adaptive_focus
    ...
    def exercise(self,mode):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with contextlib.ExitStack() as stack:
                stack.enter_context(patch.object(adaptive_focus,'acquire_foreground',side_effect=TimeoutError('focus timeout') if mode=='focus_failed' else None,return_value=dict(acquired=True,pid=123,activationCalls=1,lastActivationUtc='2026-09-08T01:00:00Z',acquiredUtc='2026-09-08T01:00:00Z')))
                stack.enter_context(patch.object(m,'validate_plan',return_value=plan))
                stack.enter_context(patch.object(adaptive_recovery,'validate_policy',return_value=dict(resumeIndex=0)))
