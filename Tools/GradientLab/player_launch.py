"""Serial Gradient Player dispatch with durable intents and exact owned PIDs.
    ...
    try:
        for run in plan["runs"]:
            ...
            if directory.exists() or receipt_path.exists():
                ...
                receipt=read(receipt_path);need(receipt.get("schemaVersion")=="xuilab.gradient.launch-receipt/v1" and receipt.get("runId")==run_id and type(receipt.get("exitCode")) is int and receipt["exitCode"]==0 and receipt["planSha256"]==plan_hash and receipt["buildManifestSha256"]==build_hash,"Receipt run/exit/plan/build mismatch")
                ...
                print(json.dumps({"runId":run_id,"action":"verified_existing","qualityStatus":result["qualityStatus"]}),flush=True);continue
            need(not log.exists(),f"{run_id}: existing log without terminal receipt")
            log.parent.mkdir(exist_ok=True)
            marker={"schemaVersion":"xuilab.gradient.launch-intent/v1","runId":run_id,"planSha256":plan_hash,"buildManifestSha256":build_hash,"token":token,"ownerPid":os.getpid(),"player":str(player),"createdUtc":dt.datetime.now(dt.timezone.utc).isoformat()}
            ...
            try:
                ...
                contracts.write_json_new(receipt_path,{"schemaVersion":"xuilab.gradient.launch-receipt/v1","runId":run_id,"planSha256":plan_hash,"buildManifestSha256":build_hash,"pid":process.pid,"exitCode":exit_code,"durationSeconds":time.monotonic()-started,"identitySha256":contracts.sha256_file(directory/"identity.json"),"qualityStatus":result["qualityStatus"],"completedUtc":dt.datetime.now(dt.timezone.utc).isoformat()})
                remove_owned(intent,token)
                print(json.dumps({"runId":run_id,"action":"completed","p95":detail["frameP95"],"qualityStatus":result["qualityStatus"]}),flush=True)
