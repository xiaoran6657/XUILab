...
from adaptive_continue import verify_receipt as continued_receipt
...
def verify(plan_path,run_root,repo,gate_path,build_path,policy_path,policy_hash):
    ...
    selected=plan["runs"][policy["resumeIndex"]:]
    ids={r["runId"] for r in selected}
    need({p.name for p in run_root.iterdir()}==ids|{".receipts",".logs",".launches"},"Incomplete exact continuation tree")
    for folder,suffix in ((".receipts",".json"),(".logs",".log"),(".launches",".json")):
        need({p.name for p in (run_root/folder).iterdir()}=={name+suffix for name in ids},"Incomplete continuation sidecars")
    locations={r["runId"]:(run_root,policy_hash,continued_receipt) for r in selected}
    for prior in policy["history"]:
        verifier=verify_receipt if prior["start"]==0 else continued_receipt
        for r in plan["runs"][prior["start"]:prior["end"]]:locations[r["runId"]]=(repo/prior["root"],prior["policySha256"],verifier)
    need(set(locations)=={r["runId"] for r in plan["runs"]},"Campaign coverage")
    rows=[];environment=None
    for run in plan["runs"]:
        source,dispatch_hash,receipt_verifier=locations[run["runId"]]
        receipt=receipt_verifier(source,run,ph,bh,player,manifest)
        need(receipt.get("dispatchPolicySha256")==dispatch_hash,"Receipt dispatch policy")
        result,detail=verify_run(source/run["runId"],plan,run,ph,repo,gate)
