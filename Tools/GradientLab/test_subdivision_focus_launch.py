...
import subdivision_focus_launch as m
import subdivision_campaign
import startup_focus
    ...
    def exercise(self,mode):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            def result(*args):
                ...
                (d/'events.log').write_text('2026-09-08T01:00:01Z state=Prepare\n')
            ...
            with contextlib.ExitStack() as stack:
                stack.enter_context(patch.object(startup_focus,'acquire_foreground',side_effect=TimeoutError('focus timeout') if mode=='focus_failed' else None,return_value=dict(acquired=True,pid=123,activationCalls=1,lastActivationUtc='2026-09-08T01:00:00Z',acquiredUtc='2026-09-08T01:00:00Z')))
                stack.enter_context(patch.object(m,'validate_plan',return_value=plan))
                stack.enter_context(patch.object(subdivision_campaign,'validate_policy',return_value=dict(resumeIndex=0)))
            ...
            self.assertEqual((root/'r-orchestration-failure.json').exists(),mode in ('timeout','focus_failed'))
            if mode=='success':
            ...
            if mode in ('timeout','focus_failed'):proc.kill.assert_called_once()
    def test_focus_failed(self):self.exercise('focus_failed')
