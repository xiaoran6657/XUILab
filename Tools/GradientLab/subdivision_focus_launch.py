...
def verify_startup_focus(focus,directory,pid):
    need(focus.get("acquired") is True and focus.get("pid")==pid,"Startup focus receipt")
    need(type(focus.get("activationCalls")) is int and focus["activationCalls"]>=0,"Focus activation count")
    first=(directory/"events.log").read_text(encoding="utf-8").splitlines()[0]
    need(first.endswith("state=Prepare"),"Prepare timestamp missing")
    def timestamp(value):return dt.datetime.fromisoformat(value.replace("Z","+00:00"))
    acquired=timestamp(focus["acquiredUtc"]);prepare=timestamp(first.split()[0])
    need(acquired<=prepare,"Focus acquisition was not completed before Prepare")
    if focus["activationCalls"]:
        need(timestamp(focus["lastActivationUtc"])<=acquired,"Focus activation after acquisition")
    else:need(focus["lastActivationUtc"] is None,"Unexpected focus mutation")
    ...
    try:
        ...
        for run in plan["runs"][policy["resumeIndex"]:]:
            ...
            if directory.exists() or receipt_path.exists():
                ...
                need(receipt.get("recoveryPolicySha256")==policy_hash,"Existing receipt policy mismatch")
            ...
            try:
                ...
                from startup_focus import acquire_foreground
                try:
