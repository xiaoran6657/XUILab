...
| --- | --- | --- | --- |
| 为什么刷新一项却Bind九项？ | [列表](LIST_REFRESH.md) | [旧基线诊断](../Experiments/LIST_REFRESH_BASELINE.md) | [trace.json（历史记录未公开）](../Showcase/HISTORICAL_RECORDS.md)，找name=middle/backend=normal |
...
| 改进到底有多大？ | [列表](LIST_REFRESH.md) | [列表结果](../Experiments/LIST_REFRESH_RESULTS.md) | [逐轮比较表（历史记录未公开）](../Showcase/HISTORICAL_RECORDS.md)与[复算明细（历史记录未公开）](../Showcase/HISTORICAL_RECORDS.md) |
...
| 固定和自适应为什么结论不同？ | [渐变](GRADIENT_SUBDIVISION.md) | [固定段结果](../Experiments/GRADIENT_SUBDIVISION_RESULTS.md)、[自适应结果](../Experiments/GRADIENT_ADAPTIVE_RESULTS.md) | [自适应矩阵报告（历史记录未公开）](../Showcase/HISTORICAL_RECORDS.md) |
...
2. 打开[计划（历史记录未公开）](../Showcase/HISTORICAL_RECORDS.md)，定位同场景与轮次，取得真实run-id与配置；不要凭数字顺序猜目录。
3. 在[复算明细（历史记录未公开）](../Showcase/HISTORICAL_RECORDS.md)按runId定位，读frameP95、bindDelta、environment与指标能力；结合[比较表（历史记录未公开）](../Showcase/HISTORICAL_RECORDS.md)确认汇总口径。
4. 在[运行根目录（历史记录未公开）](../Showcase/HISTORICAL_RECORDS.md)定位该run-id下的config、identity、samples、summary、日志。字段结构以实际JSON/CSV为准，不能假定所有实验文件完全同schema。
...
对自适应渐变，先读[最终报告（历史记录未公开）](../Showcase/HISTORICAL_RECORDS.md)的comparisons：baselineP95、variantP95、pairedDeltas、thresholdMs、result、scenario；再根据报告及[实验说明](../Experiments/GRADIENT_ADAPTIVE_RESULTS.md)定位matrix/matrix-recovery1/matrix-recovery2三个有效来源。五轮成对数据不可只取较快一轮。
...
[trace.json（历史记录未公开）](../Showcase/HISTORICAL_RECORDS.md)的schema为xuilab.list.refresh-trace/v1，candidate为list-refresh-trace-r1。挑选traces中name=middle、backend=normal：target=14、totalBinds=9、targetBinds=1、scannedCells=1000。它观察旧M1 Runtime，通过反射替换内部数据后调用原RefreshItem；不能假装是当前UpdateItem公开API的运行。

[RefreshTraceTests.cs](../../XUILab/Assets/XUILab/ListLab/Tests/PlayMode/RefreshTraceTests.cs)是诊断入口；读mutation说明、snapshots.phase、累计计数与前后差值、事件index、清理断言。[trace复算结果（历史记录未公开）](../Showcase/HISTORICAL_RECORDS.md)与[refresh_trace_verify.py](../../Tools/ListLab/refresh_trace_verify.py)说明如何核验。不要重新运行旧诊断来覆盖同名历史输出。

旧源码可从[冻结基线（历史记录未公开）](../Showcase/HISTORICAL_RECORDS.md)或下表历史归档读取；阅读时使用冻结副本，活动Unity工程仍只有仓库XUILab/，不在副本启动另一个Editor。当前源码用于理解新行为，冻结源码用于解释旧观测。
...
| 所需材料 | 入口 | 身份/说明 |
| --- | --- | --- |
| M3最终源码、干净Development构建与57项回归 | [最终包（历史记录未公开）](../Showcase/HISTORICAL_RECORDS.md) | source399a4fc，clean源码；构建实际源019d668；不包含本次新Learn文档 |
| 可信包哈希与验收回执 | [最终归档回执（历史记录未公开）](../Showcase/HISTORICAL_RECORDS.md) | 包索引SHA以回执为准，不根据可修改index自己宣称可信 |
| M2/M3历史实验、图与失败尝试 | [历史包（历史记录未公开）](../Showcase/HISTORICAL_RECORDS.md) | b40eac4+dirty历史身份，保留原始样本 |
| 更早M1/INFRA资料 | [旧归档（历史记录未公开）](../Showcase/HISTORICAL_RECORDS.md) | 根据其index和[M1证据索引（历史记录未公开）](../Showcase/HISTORICAL_RECORDS.md)读取 |
| 三个包的交接关系 | [M3本地检查点（历史记录未公开）](../Showcase/HISTORICAL_RECORDS.md) | 最终包与历史包是配套交接，不能用短smoke替代正式历史矩阵 |

历史包内的extra根：list-trace、list-refresh、gradient-baseline、gradient-fixed、gradient-adaptive。其内通常保留XUILab/、Tools/、Docs/、Artifacts/等相对结构；先读索引找到具体成员，不能仅把原路径字符串机械替换。当前[冻结列表刷新（历史记录未公开）](../Showcase/HISTORICAL_RECORDS.md)、[固定段（历史记录未公开）](../Showcase/HISTORICAL_RECORDS.md)、[自适应（历史记录未公开）](../Showcase/HISTORICAL_RECORDS.md)也是可直接阅读入口。
