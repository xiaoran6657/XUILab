...
- candidate: gradient-contract-r3
- brief_revision: r3
...
细分输入 vertexCount 必须恰好为4、indexCount恰好为6，不允许额外未引用顶点。公开颜色API对任一非有限通道拒绝且不改变状态；有限越界颜色逐通道clamp到[0,1]。序列化OnValidate对非有限颜色通道固定恢复0，对有限通道clamp到[0,1]。这两项澄清与既有实现相同，不改变曲线或Mesh输出。
