...
参考与核对日期：2026-09-05。本页记录设计时的依据；环境和进度描述属于历史观测，当前状态读取 [PM 入口](../PM/PROJECT_STATUS.md)。规则中面向 XUILab 的风险划分、角色组合、记录粒度和性能协议是项目设计决定，不是声称官方要求使用这套流程。
...
| 已运行任务和证据必须绑定版本 | 保留，并兼容无首个提交的探索阶段 | 设计时仓库尚无提交；正式证据仍遵守路线图冻结要求 |
