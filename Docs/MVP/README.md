...
6. 文档／PM／工具基础设施使用 `INFRA-NNN` 与 [PM 合同](../Agents/PM_CONTRACT.md)，不加入 MVP 功能任务表。阶段页的当前状态采用 PM 链接；历史协议版本与实验报告保留原身份，不追写为新状态。

稳定条款、任务依赖与证据种类索引见[跨阶段验收映射](ACCEPTANCE_MAP.md)；动态状态仍由 PM 维护。
