...
完整本地快照使用项目现有[源码归档工具](../Agents/SOURCE_ARCHIVE.md)，位置为`Artifacts/m4-local-archive-r1/`。它是用于保存/审查的目录副本，不是第二个活动Unity工程。实际归档文件数、index SHA256和检查终态以[归档回执](../../Artifacts/m4-archive-receipt-r1.json)为准；仅目录存在不等于成功。
...
恢复到新工作区时，将source/内容放回仓库根；按[恢复映射](../../Artifacts/m4-archive-inputs-r2/restore-map.json)把extra放回对应Artifacts相对路径。只打开恢复后的XUILab/。原receipt保留原机器路径，不重写成新路径来假装当时在那里运行；历史核对遵循[复跑说明](REPLAY.md)，新运行使用新的身份和目录。
